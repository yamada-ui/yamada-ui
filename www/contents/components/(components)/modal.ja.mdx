...
:::tip
`Modal`は、ユーザーがページへ戻る前に短い操作や判断へ集中する必要がある場合に使用します。より多くのページ文脈を保つ操作やサイドパネルとして表示したい場合は、[Drawer](/docs/components/drawer)を使用します。
:::
...
配置を変更する場合は、`placement`に`"start-center"`や`"center-start"`などを設定します。デフォルトでは、`"center-center"`が設定されています。
...
| 要素                                             | ロールと属性             | 使い方                                                                                              |
| ------------------------------------------------ | ------------------------ | --------------------------------------------------------------------------------------------------- |
| `section.ui-modal__content`                      | `role="dialog"`          | ダイアログであることを示します。                                                                    |
|                                                  | `aria-modal="true"`      | 表示されているウィジェットがモーダルであることを示します。                                          |
|                                                  | `aria-labelledby`        | `h2.ui-modal__title`の`id`を設定します。                                                            |
|                                                  | `aria-describedby`       | `div.ui-modal__body`の`id`を設定します。                                                            |
| `Modal.OpenTrigger (.ui-modal__trigger--open)`   | `aria-haspopup="dialog"` | ダイアログが存在することを示します。                                                                |
|                                                  | `aria-expanded`          | モーダルが開いている場合は`"true"`を設定し、閉じている場合は`"false"`を設定します。                 |
|                                                  | `aria-controls`          | モーダルが開いている場合は、`section.ui-modal__content`の`id`を設定します。                         |
|                                                  | `aria-label`             | `"モーダルを開く"`を設定します。レンダリングされるタグは`asChild`でマージされる子要素に従います。   |
| `button.ui-modal__close-button`                  | `aria-label`             | `"モーダルを閉じる"`を設定します。                                                                  |
| `Modal.CloseTrigger (.ui-modal__trigger--close)` | `aria-label`             | `"モーダルを閉じる"`を設定します。レンダリングされるタグは`asChild`でマージされる子要素に従います。 |
| `div.ui-modal__overlay`                          | `aria-hidden`            | 要素をアクセシビリティツリーから除外します。                                                        |
