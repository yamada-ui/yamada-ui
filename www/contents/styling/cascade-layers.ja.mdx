...
Yamada UIは、CSSの[カスケードレイヤー](https://developer.mozilla.org/ja/docs/Web/CSS/@layer)を使用して、[テーマ](/docs/theming)と[Style Props](/docs/styling/style-props)に優先順位を設定しています。この優先順位は、コンポーネントのスタイリングをする上で重要な役割を果たします。
...
- `base`: コンポーネントの[ベーススタイル](/docs/components/styled#base-style)。
- `size`: コンポーネントの[サイズスタイル](/docs/components/styled#size-style)。
- `variant`: コンポーネントの[バリアントスタイル](/docs/components/styled#variant-style)。
- `props`: コンポーネントの[プロップススタイル](/docs/components/styled#props-style)。
- `compounds`: コンポーネントの[コンポジットスタイル](/docs/components/styled#compounds-style)。
