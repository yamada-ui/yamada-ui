...
Here is an example of using a basic calendar.
...
To change the date format, set strings to `dateFormat` or `yearFormat`.
...
To customize the label button, set `props` to `labelProps`.
...
To customize control buttons, set `props` to `controlProps`, `prevProps`, or `nextProps`.
