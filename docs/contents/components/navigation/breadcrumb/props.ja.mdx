---
title: Breadcrumb
description: "`Breadcrumb`は、ユーザーがウェブサイトの階層を理解するのに役立つコンポーネントです。"
package_name: "@yamada-ui/breadcrumb"
with_description: true
tab: Props
order: 1
---

## Props

### BreadcrumbEllipsisProps

<PropsCard
  id="breadcrumbellipsis-size"
  name="size"
  type='UIValue<number | "-moz-initial" | "inherit" | "initial" | "revert" | "revert-layer" | "unset" | "large" | "medium" | "small" | "x-large" | "x-small" | "xx-large" | "xx-small" | "xxx-large" | ... 17 more ... | "xs">'
  description={"The CSS `font-size` property."}
  deprecated={"Use `fontSize` instead."}
/>

### BreadcrumbItemProps

<PropsCard
  id="breadcrumbitem-gap"
  name="gap"
  type='UIValue<number | "px" | "-moz-initial" | "inherit" | "initial" | "revert" | "revert-layer" | "unset" | (string & {}) | "2xl" | "lg" | "md" | "sm" | "xl" | ({} & string) | "3xl" | "4xl" | ... 100 more ... | "normal">'
  description={"The left and right margin applied to the divider."}
  defaultValue={"'2'"}
/>

<PropsCard
  id="breadcrumbitem-iscurrentpage"
  name="isCurrentPage"
  type="boolean"
  description={"If `true`, change to span element."}
  defaultValue={"false"}
/>

<PropsCard
  id="breadcrumbitem-islastchild"
  name="isLastChild"
  type="boolean"
  description={"If `true`, not show divider."}
  defaultValue={"false"}
/>

<PropsCard
  id="breadcrumbitem-divider"
  name="divider"
  type="type ONLY_FOR_FORMAT =
  | string
  | ReactElement<any, string | JSXElementConstructor<any>>"
  description={"The visual divider between each breadcrumb item."}
  defaultValue={"'/'"}
/>

### BreadcrumbLinkProps

<PropsCard
  id="breadcrumblink-iscurrentpage"
  name="isCurrentPage"
  type="boolean"
  description={"If `true`, change to span element."}
  defaultValue={"false"}
/>

### BreadcrumbProps

<PropsCard
  id="breadcrumb-ellipsis"
  name="ellipsis"
  type="string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | ((props: { ...; }) => ReactNode)"
  description={"The icon to be used in the ellipsis."}
/>

<PropsCard
  id="breadcrumb-endboundaries"
  name="endBoundaries"
  type="UIValue<number>"
  description={"Number of elements visible on the end(right) edges."}
/>

<PropsCard
  id="breadcrumb-gap"
  name="gap"
  type='UIValue<number | "px" | "-moz-initial" | "inherit" | "initial" | "revert" | "revert-layer" | "unset" | (string & {}) | "2xl" | "lg" | "md" | "sm" | "xl" | ({} & string) | "3xl" | "4xl" | ... 100 more ... | "normal">'
  description={"The left and right margin applied to the divider."}
  defaultValue={"'2'"}
/>

<PropsCard
  id="breadcrumb-items"
  name="items"
  type="BreadcrumbGenerateItem[]"
  description={"If provided, generate breadcrumb items based on items."}
/>

<PropsCard
  id="breadcrumb-listprops"
  name="listProps"
  type='HTMLUIProps<"ol">'
  description={"Props for ol element."}
/>

<PropsCard
  id="breadcrumb-divider"
  name="divider"
  type="type ONLY_FOR_FORMAT =
  | string
  | ReactElement<any, string | JSXElementConstructor<any>>"
  description={"The visual divider between each breadcrumb item."}
  defaultValue={"'/'"}
/>

<PropsCard
  id="breadcrumb-startboundaries"
  name="startBoundaries"
  type="UIValue<number>"
  description={"Number of elements visible on the start(left) edges."}
/>

### BreadcrumbDividerProps

<PropsCard
  id="breadcrumbdivider-gap"
  name="gap"
  type='number | "px" | "-moz-initial" | "inherit" | "initial" | "revert" | "revert-layer" | "unset" | (string & {}) | "2xl" | "lg" | "md" | "sm" | "xl" | ({} & string) | "3xl" | "4xl" | "xs" | ... 104 more ... | ((theme: StyledTheme) => UIValue<...>)'
  description={
    "The CSS `margin-inline-start`, and `margin-inline-end` property."
  }
/>
