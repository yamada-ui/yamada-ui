...
### Handle Parsing of Entered Value

To handle the parsing of the entered value, use `parseDate`.
...
### Allow Input Beyond Min and Max Range
...
:::note
For locale, please check [here](https://day.js.org/docs/en/i18n/instance-locale).
:::
...
:::note
For format, please check [here](https://day.js.org/docs/en/display/format#list-of-localized-formats).
:::
...
By default, the calendar automatically closes on selection. To prevent the calendar from closing on selection, set `closeOnSelect` to `false`.
...
If you want to change the position of the tooltip depending on the size of the element or the situation, adjust the position with `gutter` or `offset`.
