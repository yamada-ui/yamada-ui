...
type Option = { out?: string; lang?: keyof typeof LANG_MAP }
...
program
  ...
  .option('-o, --out <path>')
  .option('-l, --lang <lang>')
  .action(async (targetPath: string, { out: outPath, lang: targetLang = 'en' }: Option) => {
    ...
    try {
      ...
      spinner.text = `Translate files [0 / ${totalCount}]...`
      ...
      if (successes.size !== 0) {
        console.log(
          ...
          '\n',
          chalk.green(
            ...Array.from(successes.entries()).map(([path, duration]) => `${path}: ${duration}s\n`),
          ),
        )
      }
      ...
      if (errors.size !== 0) {
        console.log(
          ...
          '\n',
          chalk.red(
            ...Array.from(errors.entries()).map(([path, duration]) => `${path}: ${duration}s\n`),
          ),
        )
      }
    } catch (e) {
