...
To use a loading animation, set `isLoading` to `true`. If you want to change the text during loading, use `loadingText`.
...
To group buttons, use `ButtonGroup`. `ButtonGroup` allows you to set `variant`, `size`, `colorScheme`, `isDisabled` for child elements (`Button`) in bulk.
