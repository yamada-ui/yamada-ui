...
async function loadGoogleFont(
  ...
  const url = `https://fonts.googleapis.com/css2?family=${name}:wght@${weight}&text=${encodeURIComponent(text)}`
  const css = await (await fetch(url)).text()
  const resource = css.match(/src: url\((.+)\) format\('(opentype|truetype)'\)/)

  if (!resource) throw new Error("failed to load font data")

  const response = await fetch(resource[1] as string)

  if (response.status !== 200) throw new Error("failed to load font data")
...
export async function GET(request: Request) {
  ...
  const fonts = await Promise.all([
    ...
    loadGoogleFont("Noto+Sans+JP", 600, title + "…"),
    loadGoogleFont("Inter", 400, description + "…"),
    loadGoogleFont("Inter", 600, title + "…"),
  ])
