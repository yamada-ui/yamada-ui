...
ローカルストレージに値を保存する場合は、`Resizable.useLayout`を使用します。

:::warning
保存された各値と`Resizable.Item`を関連付けするために`Resizable.Item`に`id`を設定する必要があります。
:::
...
`cookies`に値を保存する場合は、`Resizable.useLayout`を使用します。
