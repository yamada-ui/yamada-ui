...
`SegmentedControl.Item`にアイコンのみを使用する場合は、`SegmentedControl.Item`の`inputProps`に`aria-label`を設定します。
...
| `SegmentedControl.Item > input` | `aria-disabled`     | `disabled`が設定されている場合は、項目のラベルと非表示のラジオ入力に`"true"`を設定します。                |
|                                 | `readOnly`          | `readOnly`が設定されている場合は、非表示のラジオ入力にネイティブの`readOnly`属性を設定します。            |
|                                 | `data-readonly`     | `readOnly`が設定されている場合は、項目のラベルと非表示のラジオ入力に設定します。                          |
