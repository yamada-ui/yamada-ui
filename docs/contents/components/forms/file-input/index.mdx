...
To use a custom component, set `component` to a `ReactElement`. `component` will provide `value` (`File`) and `index`.
...
To format file names, use `format`. `format` will provide `File`.
