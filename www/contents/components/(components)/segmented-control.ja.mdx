...
`SegmentedControl.Item`にアイコンのみを使用する場合は、`inputProps`を使用して非表示のラジオ入力に`aria-label`を設定します。
...
| `SegmentedControl.Item` | `aria-disabled`     | `disabled`が設定されている場合は、項目のラベルと非表示のラジオ入力に`"true"`を設定します。                |
