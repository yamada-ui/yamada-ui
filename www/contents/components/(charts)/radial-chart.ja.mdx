...
```tsx preview functional
const series = useMemo<RadialChart.RadialProps<PolarChartData>[]>(
  () => [{ dataKey: "visits", nameKey: "browser" }],
  [],
)
const data = useMemo(() => RadialChart.mergeData(createPolarChartData()), [])
...
| キー         | 説明                                                                                       | 状態                 |
| ------------ | ------------------------------------------------------------------------------------------ | -------------------- |
| `ArrowRight` | チャート面にフォーカスを維持したまま、次のデータポイントのツールチップを表示します。       | `withTooltip={true}` |
| `ArrowLeft`  | チャート面にフォーカスを維持したまま、前のデータポイントのツールチップを表示します。       | `withTooltip={true}` |
| `Enter`      | チャートにフォーカスを維持したまま、アクティブなデータポイントのツールチップを切り替えます。 | `withTooltip={true}` |
...
| `svg`          | `tabIndex={0}`       | キーボードでチャート面にフォーカスできるようにします。 |
