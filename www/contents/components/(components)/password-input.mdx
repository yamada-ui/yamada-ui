...
| --------------- | ------------------ | ------------------------------------- |
| `PasswordInput` | `aria-invalid`     | Set to `"true"` if `invalid` is set.  |
| `PasswordInput` | `aria-disabled`    | Set to `"true"` if `disabled` is set. |
| `PasswordInput` | `aria-readonly`    | Set to `"true"` if `readOnly` is set. |
| `PasswordInput` | `aria-required`    | Set to `"true"` if `required` is set. |
| `StrengthMeter` | `role="meter"`     | Indicates that this is a meter.       |
| `StrengthMeter` | `aria-label`       | Sets `"Password strength meter"`.     |
| `StrengthMeter` | `aria-valuemin`    | Sets the `0` value.                   |
| `StrengthMeter` | `aria-valuemax`    | Sets the `max` value.                 |
| `StrengthMeter` | `aria-valuenow`    | Sets the current value.               |
