...
To change the placement, set `placement` to values like `top` or `left-start`. By default, `bottom` is set.
...
By default, the popover is activated when the `PopoverTrigger` element is clicked. If you want it to activate on hover, set `trigger` to `hover`.
...
By default, it is rendered in the DOM regardless of being active. In other words, it is rendered even when not visible, but it is hidden by style.
...
If you want to customize the `PopoverCloseButton` by passing `props` or similar, set it without omitting.
