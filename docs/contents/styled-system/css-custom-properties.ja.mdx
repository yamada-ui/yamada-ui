...
[Style Props](/styled-system/style-props)の`var`を使用して、[CSSカスタムプロパティ(変数)](https://developer.mozilla.org/ja/docs/Web/CSS/Using_CSS_custom_properties)を作成します。
...
:::note status=warning
`var`で設定したカスタムプロパティは、設定した要素の子要素にのみ適用されます。
:::
