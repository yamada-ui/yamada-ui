...
```tsx preview
<Center bg="mix(red.500, blue.500)" w="full" height="2xs" p="md" color="white">
...
```tsx preview
<Center bg="shade(yellow.500, 50%)" w="full" height="2xs" p="md" color="white">
  Shade color
</Center>
```
...
:::note
A shorthand notation like `bg="red.500 / 50"` is also available.
:::
...
```tsx preview
<Box w="full" h="2xs" bgGradient="linear(to-r, green.200, pink.500)" />
```
...
```tsx preview
<Box w="full" h="2xs" bgGradient="linear(to-r, #59a9e1, #f37bdf)" />
```
