...
```tsx live=false title=_app.tsx highlight={6}
...
<Alert status='info'>
  <AlertIcon />
  App Routerの場合は、[こちら](/docs/getting-started/frameworks/nextjs/app-router#uiprovidercacheprovidercolormodescriptを追加する)をご覧ください。
</Alert>
...
`UIProvider`の`colorModeManager`に`cookieStorageSSR`を渡します。
