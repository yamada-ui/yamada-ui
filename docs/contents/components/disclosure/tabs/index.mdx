...
- `Tabs`: A wrapper component that controls child elements (`TabList`, `TabPanels`).
- `TabList`: A wrapper component that controls child elements (`Tab`).
- `Tab`: A component that functions as a label for the panel and a button to display that panel.
- `TabPanels`: A wrapper component that controls child elements (`TabPanel`).
...
### Activate Tabs from the Start

To activate a specific tab from the beginning, set the `index` of the item to `defaultIndex`.
...
### Stretch Tabs to Container Width
...
If you want to delay the rendering of each panel until the tab is active, set `isLazy` to `true`.
