...
describe("<Table />", () => {
  ...
  describe("pagination", () => {
    ...
    test("changes page through PageDown / PageUp keyboard navigation", async () => {
      ...
      const { user } = render(<PaginatedTable />)

      const firstCell = getTable().querySelector(
      ...
      expect(getTable().querySelectorAll("tbody tr")).toHaveLength(2)
    ...
    test("updates `tabIndex` on the focused cell after page change", async () => {
      ...
      const cell = getTable().querySelector(
        '[data-colindex="0"][data-rowindex="0"]',
      )
  ...
  describe("keyboard navigation", () => {
    test("renders cells with `tabindex` only when `enableKeyboardNavigation` is true", () => {
      const { rerender } = render(
        <Table
          columns={columns}
          data={data}
    ...
    test("does not move focus on arrow keys when keyboard navigation is disabled", async () => {
      const { user } = render(
        <Table
          columns={columns}
          data={data}
      ...
      const firstTh = getTable().querySelector("th")
    ...
    test("does nothing for unhandled keys", async () => {
      const { user } = render(
        <Table
          columns={columns}
          data={data}
    ...
    test("ignores arrow keys at table boundaries without crashing", async () => {
      const { user } = render(
        <Table
          columns={columns}
          data={data}
      ...
      const lastTh = getTable().querySelector(
      ...
      const lastCell = getTable().querySelector(
        `[data-colindex="0"][data-rowindex="${data.length}"]`,
      )
    ...
    test("ArrowDown / ArrowRight navigates through grouped headers", async () => {
      const { user } = render(
        <Table
    ...
    test("ArrowRight navigates through deeply nested grouped headers", async () => {
      const { user } = render(
        <Table
    ...
    test("focuses an inner `[data-focusable]` element when present", () => {
      render(
        <Table
          columns={columns}
          data={data}
      ...
      expect(firstTh?.querySelector("[data-focusable]")).toBeInTheDocument()
    })

    test("ArrowDown advances to the next data row", async () => {
      const { user } = render(
        <Table
          columns={columns}
          data={data}
    ...
    test("focus event on a non-initial cell sets `tabindex` to 0", async () => {
      const { user } = render(
        <Table
          columns={columns}
          data={data}
    ...
    test("navigates from header through body to footer", async () => {
      const { user } = render(
        <Table
          columns={columns}
          data={data}
