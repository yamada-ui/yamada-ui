...
::::tip
縦方向または横方向の積み重ねを既定のスタイル付きで使いたい場合は、[VStack](/docs/components/v-stack)または[HStack](/docs/components/h-stack)の使用を検討してください。`VStack`はデフォルトで縦方向に並び、子要素が伸長されて幅いっぱいに表示されます。`HStack`はデフォルトで横方向に並び、交差軸方向で中央揃えになります。
::::
