...
To change the speed of slide transition, set `speed` to a number. By default, `10` is set.
...
To change the size of slides, set `slideSize` to a string or a number. By default, `100%` is set.
...
### Change the Position of Slides

To change the position of slides, set `align` to `start`, `center`, `end`, or a number. By default, `center` is set.
...
#### Change the Interval of Autoplay

To change the interval of autoplay, set `delay` to a number (milliseconds). By default, `4000` is set.
...
To disable the loop, set `loop` to `false`.
