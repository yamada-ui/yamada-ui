...
:::note status=warning
`custom`は、[デフォルトのコンフィグ](/docs/styled-system/configure/default-config)で設定されていません。カスタムローディングを設定したい場合は、[こちら](#カスタムコンポーネントを使う)をご覧ください。
:::
...
return (
  <Wrap gap='md'>
    <Button colorScheme='gray' onClick={() => screen.start({ duration: 5000 })}>
      Start screen loading
    </Button>
    <Button colorScheme='gray' onClick={() => page.start({ duration: 5000 })}>
      Start page loading
    </Button>
    <Button colorScheme='gray' onClick={() => background.start({ duration: 5000 })}>
      Start background loading
    </Button>
  </Wrap>
)
...
return (
  <Wrap gap='md'>
    <Button colorScheme='gray' onClick={() => screen.start({ message: 'Loading', duration: 5000 })}>
      Start screen loading
    </Button>
    <Button colorScheme='gray' onClick={() => page.start({ message: 'Loading', duration: 5000 })}>
      Start page loading
    </Button>
    <Button
      colorScheme='gray'
...
return (
  <Wrap gap='md'>
    <Button
      colorScheme='gray'
      onClick={() =>
        screen.start({ message: <Text color='primary'>Loading</Text>, duration: 5000 })
      }
    >
      Start screen loading
    </Button>
    <Button
      colorScheme='gray'
      onClick={() => page.start({ message: <Text color='primary'>Loading</Text>, duration: 5000 })}
    >
      Start page loading
    </Button>
    <Button
      colorScheme='gray'
      onClick={() =>
        background.start({ message: <Text color='primary'>Loading</Text>, duration: 5000 })
      }
    >
      Start background loading
    </Button>
...
function App() {
  ...
  return (
    <Button colorScheme='gray' onClick={() => custom.start({ duration: 5000 })}>
