...
<Alert status='warning'>
  <AlertIcon />
  テーマをカスタマイズする場合は、`extendTheme`を使用することをオススメします。これは、[デフォルトのテーマ](/docs/styled-system/theming/default-theme)を継承したコンフィグを作成します。`extendTheme`を使用しない場合は、[デフォルトのテーマ](/docs/styled-system/theming/default-theme)を継承しないので、全く新しいコンフィグになります。
</Alert>
