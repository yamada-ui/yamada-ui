---
title: Pagination
description: "`Pagination`は、コンテンツのページ分割とナビゲーションを管理するためのコンポーネントです。"
tags: ["navigation", "selection"]
storybook: components-pagination--basic
source: components/pagination
style: components/pagination/pagination.style.ts
---

```tsx preview
<Pagination.Root total={10} />
```

## 使い方

:::code-group

```tsx [package]
import { Pagination } from "@yamada-ui/react"
```

```tsx [alias]
import { Pagination } from "@/components/ui"
```

```tsx [monorepo]
import { Pagination } from "@workspaces/ui"
```

:::

```tsx
<Pagination.Root total={10} />
```

### バリアントを変更する

```tsx preview
<VStack>
  <For each={["solid", "subtle", "surface", "outline"]}>
    {(variant) => (
      <Pagination.Root key={variant} variant={variant} total={10} />
    )}
  </For>
</VStack>
```

### サイズを変更する

```tsx preview
<VStack>
  <For each={["xs", "sm", "md", "lg", "xl"]}>
    {(size) => <Pagination.Root key={size} size={size} total={10} />}
  </For>
</VStack>
```

### カラースキームを変更する

```tsx preview
<VStack>
  <For each={["success", "warning"]}>
    {(colorScheme) => (
      <Pagination.Root key={colorScheme} colorScheme={colorScheme} total={10} />
    )}
  </For>
</VStack>
```

### デフォルトのページを設定する

```tsx preview
<Pagination.Root defaultPage={5} total={10} />
```

### エッジコントロールボタンを追加する

最初と最後のページに移動するボタンを追加する場合は、`withEdges`を`true`にします。

```tsx preview
<Pagination.Root total={20} withEdges />
```

### 前後の数をカスタマイズする

前後の数をカスタマイズする場合は、`siblings`に数値を設定します。

```tsx preview
<Pagination.Root total={77} siblings={3} />
```

### 最初と最後の数をカスタマイズする

最初と最後の数をカスタマイズする場合は、`boundaries`に数値を設定します。

```tsx preview
<Pagination.Root total={77} boundaries={3} />
```

### 無効にする

無効にする場合は、`disabled`を`true`に設定します。

```tsx preview
<VStack>
  <For each={["solid", "outline", "ghost", "unstyled"]}>
    {(variant, index) => (
      <Pagination.Root key={index} total={10} variant={variant} disabled />
    )}
  </For>
</VStack>
```

### テキストを表示する

```tsx preview
<VStack>
  <Pagination.Root size="md" gap="md" total={10}>
    <Pagination.PrevTrigger>
      <Pagination.Item icon={<ChevronLeftIcon />} />
    </Pagination.PrevTrigger>

    <Pagination.Text format="compact" />

    <Pagination.NextTrigger>
      <Pagination.Item icon={<ChevronRightIcon />} />
    </Pagination.NextTrigger>
  </Pagination.Root>

  <Pagination.Root size="md" gap="md" total={10}>
    <Pagination.PrevTrigger>
      <Pagination.Item icon={<ChevronLeftIcon />} />
    </Pagination.PrevTrigger>

    <Pagination.Text format="short" />

    <Pagination.NextTrigger>
      <Pagination.Item icon={<ChevronRightIcon />} />
    </Pagination.NextTrigger>
  </Pagination.Root>
</VStack>
```

### コントロールボタンをカスタマイズする

```tsx preview
<Pagination.Root
  total={10}
  withEdges
  controlNextProps={{
    icon: <ArrowRightIcon />,
  }}
  controlPrevProps={{
    icon: <ArrowLeftIcon />,
  }}
  edgeEndProps={{
    icon: <ArrowRightToLineIcon />,
  }}
  edgeStartProps={{
    icon: <ArrowLeftToLineIcon />,
  }}
/>
```

### テキストをカスタマイズする

```tsx preview client
<Pagination.Root size="md" gap="md" total={20}>
  <Pagination.PrevTrigger>
    <Pagination.Item icon={<ChevronLeftIcon />} />
  </Pagination.PrevTrigger>

  <Pagination.Text>
    {({ page, total }) => `全${total}ページ中${page}ページ目`}
  </Pagination.Text>

  <Pagination.NextTrigger>
    <Pagination.Item icon={<ChevronRightIcon />} />
  </Pagination.NextTrigger>
</Pagination.Root>
```

### 制御する

```tsx preview functional client
const [page, onChange] = useState<number>(1)

return <Pagination.Root page={page} total={10} onChange={onChange} />
```

## Props

<PropsTable name="pagination" />

## アクセシビリティ

### キーボード操作

| キー             | 説明                                               | 状態 |
| ---------------- | -------------------------------------------------- | ---- |
| `Tab`            | フォーカスを次のページボタンに移動します。         | -    |
| `Shift + Tab`    | フォーカスを前のページボタンに移動します。         | -    |
| `Enter`, `Space` | フォーカスされたページボタンをアクティブにします。 | -    |

### ARIAロールと属性

| 要素                                   | ロールと属性          | 使い方                                                            |
| -------------------------------------- | --------------------- | ----------------------------------------------------------------- |
| `nav.ui-pagination__root`              | `role="navigation"`   | ページ間のナビゲーション領域であることを示します。                |
|                                        | `aria-label`          | `"ページネーション"`を設定します。                                |
| `button.ui-pagination__item`           | `aria-label`          | ページ番号ボタンの場合は`"{value}ページに移動する"`を設定します。 |
|                                        | `aria-current="page"` | 現在のページ番号ボタンに設定します。                              |
| `button.ui-pagination__item > span`    | `role="presentation"` | プレゼンテーションであることを示します。                          |
| `button.ui-pagination__trigger--prev`  | `aria-label`          | `"前のページに移動する"`を設定します。                            |
| `button.ui-pagination__trigger--next`  | `aria-label`          | `"次のページに移動する"`を設定します。                            |
| `button.ui-pagination__trigger--start` | `aria-label`          | `"最初のページに移動する"`を設定します。                          |
| `button.ui-pagination__trigger--end`   | `aria-label`          | `"最後のページに移動する"`を設定します。                          |

## 類似のコンポーネント

<Similar name="pagination" />

## 使用しているコンポーネント・フック

<Uses name="pagination" />
