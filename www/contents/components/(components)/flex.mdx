...
::::tip
If you want a preset vertical or horizontal stack instead of a general flex layout, consider using [VStack](/docs/components/v-stack) or [HStack](/docs/components/h-stack). `VStack` uses a column layout with stretched items and full width by default, while `HStack` uses a row layout with centered cross-axis alignment.
::::
