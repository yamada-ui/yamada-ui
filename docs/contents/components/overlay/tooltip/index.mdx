---
title: Tooltip
description: "`Tooltip` is a component that displays brief information, such as supplementary details about an element."
...
To change the animation for showing or hiding, set `animation` to values like `top` or `left`. By default, `scale` is set.
...
To change the duration, set `duration` to a number (milliseconds).

```tsx
<Tooltip label="へっ！きたねぇ花火だ" placement="top" openDelay={500} closeDelay={500}>
