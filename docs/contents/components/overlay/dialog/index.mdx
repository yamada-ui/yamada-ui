---
title: Dialog
description: "`Dialog` is a component used to confirm execution or interrupt actions by the user."
...
:::note
`Dialog` internally uses `Modal` and is a simplified component of `Modal`.
:::
...
Alternatively, if you do not omit `DialogHeader` or `DialogBody`, you can write it like this.
...
By default, when the dialog opens, it blocks scrolling of the main content to ensure accessibility. If you do not want to block scrolling of the main content, set `blockScrollOnMount` to `false`.
...
If you want to customize `DialogCloseButton` by passing `props`, etc., do not omit it and set it accordingly.
...
If you want to customize `DialogOverlay` by passing `props`, etc., do not omit it and set it accordingly.
...
To disable (hide) the overlay of the dialog, set `withOverlay` to `false`.
