...
`CircleProgress.Root`の`circleProps`に`aria-label`または`aria-labelledby`を設定すると、スクリーンリーダーによって読み上げられます。
...
| -------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `svg.ui-circle-progress__circle` | `role="progressbar"` | プログレスバーであることを示します。                                                                                           |
|                                  | `aria-label`         | `value`が`null`の場合（未定）は`"読み込み中..."`を、それ以外は`"18パーセント"`のように進行状況を設定します。                   |
|                                  | `aria-valuemin`      | `min`の値を設定します。デフォルトは`0`です。                                                                                   |
|                                  | `aria-valuemax`      | `max`の値を設定します。デフォルトは`100`です。                                                                                 |
|                                  | `aria-valuenow`      | 進行状況が確定している場合（`value`が`null`ではない場合）のみ現在の値を設定し、`value`が`null`の場合（未定）は設定されません。 |
