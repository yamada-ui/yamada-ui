...
tags: ["overlay", "disclosure", "floating", "animation"]
...
:::tip
Use `Modal` when users must focus on a short blocking task or decision before returning to the page. If the task should preserve more page context or use a side panel, use [Drawer](/docs/components/drawer).
:::
...
The `Modal` follows the [WAI-ARIA - Dialog (Modal) Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/) for accessibility.
...
| ------------------------------------------------ | ------------------------ | ---------------------------------------------------------------------------------- |
| `section.ui-modal__content`                      | `role="dialog"`          | Indicates that this is a dialog.                                                   |
|                                                  | `aria-modal="true"`      | Indicates that the displayed widget is modal.                                      |
|                                                  | `aria-labelledby`        | Sets to the `id` of `h2.ui-modal__title`.                                          |
|                                                  | `aria-describedby`       | Sets to the `id` of `div.ui-modal__body`.                                          |
| `Modal.OpenTrigger (.ui-modal__trigger--open)`   | `aria-haspopup="dialog"` | Indicates that a dialog exists.                                                    |
|                                                  | `aria-expanded`          | Sets to `"true"` if the modal is open, and `"false"` if it is closed.              |
|                                                  | `aria-controls`          | If the modal is open, sets to the `id` of `section.ui-modal__content`.             |
|                                                  | `aria-label`             | Sets to `"Open modal"`. The rendered tag follows the slotted child via `asChild`.  |
| `button.ui-modal__close-button`                  | `aria-label`             | Sets to `"Close modal"`.                                                           |
| `Modal.CloseTrigger (.ui-modal__trigger--close)` | `aria-label`             | Sets to `"Close modal"`. The rendered tag follows the slotted child via `asChild`. |
| `div.ui-modal__overlay`                          | `aria-hidden`            | Excludes the element from the accessibility tree.                                  |
