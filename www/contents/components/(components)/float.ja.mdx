...
::::tip
要素を辺に固定するのではなく、コンテナーの中央に配置したい場合は、[Center](/docs/components/center)の使用を検討してください。
::::

::::tip
コンテナーの辺に固定するのではなく、要素を外側にはみ出して表示したい場合は、[Bleed](/docs/components/bleed)の使用を検討してください。
::::

### 配置を変更する

配置を変更する場合は、`placement`に`start-start`や`end-center`などを指定します。
