...
:::note status=warning
Yamada UI uses `@media(max-width)` media queries for responsive design.
:::
...
:::note
If you want to change the breakpoints, please see [here](#customizing-breakpoints).
:::

## Syntax for Responsive Styles

When setting responsive styles, you can support PC-first responsive styles simply by passing an object to the style's `props`.
