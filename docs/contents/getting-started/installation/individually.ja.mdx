...
コアやコンポーネントをインストール後、アプリケーションのルートに`ThemeProvider`を追加し、`@yamada-ui/theme`から[デフォルトのテーマ](/styled-system/theming/default-theme)と[デフォルトのコンフィグ](/styled-system/configure/default-config)を渡します。もし、あなたのテーマやコンフィグがある場合は、そちらを渡します。
