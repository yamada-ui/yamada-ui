...
describe("resolveAdapter", () => {
  ...
  test("should return the named fallback when nothing is detected", async () => {
    const a = createAdapter("a", false)
    const b = createAdapter("b", false)

    await expect(resolveAdapter([a, b], "b", "/cwd")).resolves.toBe(b)
  })
  ...
  test("should throw when the explicit tool matches no adapter", async () => {
    const a = createAdapter("a", false)
    const b = createAdapter("b", "config")

    await expect(
      resolveAdapter([a, b], "a", "/cwd", "unknown"),
    ).rejects.toThrow('Tool "unknown" is not registered.')
  })
})
