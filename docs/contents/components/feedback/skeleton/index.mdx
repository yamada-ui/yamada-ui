---
title: Skeleton
description: "`Skeleton` is a component that functions as a placeholder until content is loaded."
package_name: "@yamada-ui/skeleton"
is_tabs: true
with_description: true
tab: Usage
---

## Import

```ts
import { Skeleton, SkeletonCircle, SkeletonText } from "@yamada-ui/react"
```

## Usage

```tsx
<VStack>
  <Skeleton />

  <SkeletonCircle />

  <SkeletonText />
</VStack>
```

### Change Colors

You can set the start and end colors of the animation with `startColor` and `endColor`.

```tsx
<VStack>
  <Skeleton startColor="pink.500" endColor="orange.500" />

  <SkeletonCircle startColor="pink.500" endColor="orange.500" />

  <SkeletonText startColor="pink.500" endColor="orange.500" />
</VStack>
```

### Change Size

To change the width and height of an element, set `boxSize` or `textHeight` as a string or a number.

```tsx
<VStack>
  <Skeleton h={16} />

  <SkeletonCircle boxSize={16} />

  <SkeletonText textHeight={4} />
</VStack>
```

Additionally, by passing child elements, you can match the width and height.

```tsx
<Skeleton>
  <Text>
    私の戦闘力は530000です。ですがもちろんフルパワーであなたと戦う気はありませんからご心配なく……
  </Text>
  <Text>
    私の戦闘力は530000です。ですがもちろんフルパワーであなたと戦う気はありませんからご心配なく……
  </Text>
</Skeleton>
```

### Specify Number of Lines

By setting a number to `noOfLines`, you can change the number of lines. The default is set to `3`.

```tsx
<SkeletonText noOfLines={5} />
```

### Change Duration

You can change the duration of the animation by setting `speed` as a string or a number (seconds). The default is set to `0.8`.

```tsx
<VStack>
  <Skeleton speed={2} />

  <SkeletonCircle speed={2} />

  <SkeletonText speed={2} />
</VStack>
```

### Display Content

By setting `isLoaded` to `true`, the placeholder will fade out and display the content.

```tsx functional=true
const [isLoaded, { toggle }] = useBoolean()

return (
  <VStack>
    <Button alignSelf="flex-start" onClick={toggle}>
      Toggle
    </Button>

    <SkeletonCircle isLoaded={isLoaded}>
      <Avatar name="Hirotomo Yamada" />
    </SkeletonCircle>

    <Skeleton isLoaded={isLoaded}>
      <Heading isTruncated>ギャルのパンティーおくれーーーっ！！！！！</Heading>
    </Skeleton>

    <SkeletonText isLoaded={isLoaded} noOfLines={1}>
      <Text isTruncated>
        私の戦闘力は530000です。ですがもちろんフルパワーであなたと戦う気はありませんからご心配なく……
      </Text>
    </SkeletonText>
  </VStack>
)
```

### Change Fade Out Duration

To change the duration of the placeholder's fade out, set `fadeDuration` as a number (seconds). By default, `0.4` is set.

```tsx functional=true
const [isLoaded, { toggle }] = useBoolean()

return (
  <VStack>
    <Button alignSelf="flex-start" onClick={toggle}>
      Toggle
    </Button>

    <SkeletonCircle isLoaded={isLoaded} fadeDuration={2}>
      <Avatar name="Hirotomo Yamada" />
    </SkeletonCircle>

    <Skeleton isLoaded={isLoaded} fadeDuration={4}>
      <Heading isTruncated>ギャルのパンティーおくれーーーっ！！！！！</Heading>
    </Skeleton>

    <SkeletonText isLoaded={isLoaded} fadeDuration={6} noOfLines={1}>
      <Text isTruncated>
        私の戦闘力は530000です。ですがもちろんフルパワーであなたと戦う気はありませんからご心配なく……
      </Text>
    </SkeletonText>
  </VStack>
)
```
