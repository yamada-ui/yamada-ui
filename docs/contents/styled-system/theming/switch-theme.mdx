...
Yamada UI provides a feature that allows users to switch themes.

:::note
In order to enable this feature, you need to prepare multiple themes.
:::

## Setup

In order for the user to switch themes, the following two things need to be done:
...
:::note
If you don't know how to customize a theme, please see [here](/styled-system/theming/customize-theme).
:::
...
The reason is that the theme switching is implemented using `localStorage` or `cookies`, and it is necessary to properly synchronize at the time of page loading.
...
:::note status=warning
Please pass the same `initialThemeScheme` as the config to `props`' `initialThemeScheme`.
:::
