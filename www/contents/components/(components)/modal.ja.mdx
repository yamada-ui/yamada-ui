...
表示位置を変更するには、`placement`に`start-center`や`center-start`などを設定します。デフォルトでは、`center-center`が設定されています。
...
表示または非表示のアニメーションを変更するには、`animation`に`block-start`や`inline-end`などを設定します。デフォルトでは、`scale`が設定されています。
