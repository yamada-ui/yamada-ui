...
export const Basic: Story = () => {
  ...
  return (
    <NativeAccordion.Root items={items}>
      {items.map(({ button, content }, index) => (
        <NativeAccordion.Item key={index} button={button} index={index}>
          {content}
...
export const DefaultIndex: Story = () => {
  ...
  return <NativeAccordion.Root defaultIndex={1} items={items} />
}
...
export const Toggle: Story = () => {
  ...
  return <NativeAccordion.Root items={items} toggle />
}
...
export const Multiple: Story = () => {
  ...
  return <NativeAccordion.Root defaultIndex={[1, 2]} items={items} multiple />
}
...
export const CustomLabel: Story = () => {
  return (
    <NativeAccordion.Root>
      <NativeAccordion.Item index={0}>
        <NativeAccordion.Button _expanded={{ bg: "bg.panel" }}>
          孫悟空少年編
        </NativeAccordion.Button>

        <NativeAccordion.Panel pt={3}>
          地球の人里離れた山奥に住む尻尾の生えた少年-孫悟空はある日、西の都からやって来た少女-ブルマと出会う。そこで、7つ集めると神龍（シェンロン）が現れ、どんな願いでも一つだけ叶えてくれるというドラゴンボールの存在を、さらに育ての親である孫悟飯の形見として大切に持っていた球がその1つ「四星球（スーシンチュウ）」であることを知り、ブルマと共に残りのドラゴンボールを探す旅に出る。
        </NativeAccordion.Panel>
      </NativeAccordion.Item>

      <NativeAccordion.Item index={1}>
        <NativeAccordion.Button _expanded={{ bg: "bg.panel" }}>
          ピッコロ大魔王編
        </NativeAccordion.Button>
...
export const CustomControl: Story = () => {
  ...
  const [index, onChange] = useState<
    NativeAccordion.RootProps["index"] | undefined
  >(undefined)

  return (
    <NativeAccordion.Root index={index} items={items} onChange={onChange} />
  )
}
