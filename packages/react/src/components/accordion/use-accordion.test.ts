...
function AccordionItemWrapper({ children }: PropsWithChildren) {
  const { descendants, getRootProps: _, ...context } = useAccordion()

  return createElement(
    AccordionDescendantsContext.Provider,
    { value: descendants },
    createElement(AccordionContext.Provider, { value: context }, children),
  )
}
...
describe("useAccordionItem getItemProps", () => {
  test("merges hook rest and user props via mergeProps", () => {
    const hookRef = createRef<HTMLDivElement>()
    const userRef = createRef<HTMLDivElement>()
    const hookOnClick = vi.fn()
    const userOnClick = vi.fn()
    const { result } = renderHook(
      ...
      { withProvider: false, wrapper: AccordionItemWrapper },
    )

    const merged = result.current.getItemProps({
      ref: userRef,
      className: "user",
      style: { backgroundColor: "blue" },
      onClick: userOnClick,
