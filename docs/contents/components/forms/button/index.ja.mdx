...
<Wrap gap="md">
  ...
  <Button colorScheme="secondary" variant="outline" endIcon={<ArrowRight />}>
    Button
  </Button>
...
ローディングアニメーションを使う場合は、`isLoading`を`true`にします。また、ローディング中にテキストを変更したい場合は、`loadingText`を使用します。
...
ボタンをグループ化する場合は、`ButtonGroup`を使用します。`ButtonGroup`は、子要素(`Button`)の`variant`, `size`, `colorScheme`, `isDisabled`を一括で設定できます。
