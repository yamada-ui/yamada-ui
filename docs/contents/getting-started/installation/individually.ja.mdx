...
コアやコンポーネントをインストール後、アプリケーションのルートに`ThemeProvider`を追加し、`@yamada-ui/theme`から[デフォルトのテーマ](/docs/styled-system/theming/default-theme)と[コンフィグ](/docs/styled-system/config/default-config)を渡します。もし、あなたのテーマやコンフィグがある場合は、そちらを渡します。
