...
:::tip
単一の項目のリストを表示したい場合は、[List](/docs/components/list)を使用します。
:::

:::tip
`DataList`は用語とその説明のリストを表示するために使用されます。
ヘッダーに項目名を表示したり、用語と説明以外の項目を含むリストを表示したい場合は、[NativeTable](/docs/components/native-table)の使用を検討してください。
:::

:::tip
配列の要素をループで回して、コンポーネントをレンダリングしたい場合は、[For](/docs/components/for)を使用します。
:::
