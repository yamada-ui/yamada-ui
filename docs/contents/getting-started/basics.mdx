...
description: Read a 3-minute tutorial to learn the basics of Yamada UI's themes, styles, responsiveness, dark mode, and animations.
order: 2
---

This guide will help you understand the concepts of Yamada UI. It is recommended to read this guide before starting development.

:::note status=warning
This guide is basic and is intended to make you feel the `fun` of developing with Yamada UI. Therefore, it does not explain each concept and function in depth. If you want to know more, please check the links within the page.
:::
...
:::note
The [default theme](/styled-system/theming/default-theme) is included in the `UIProvider`.
:::
...
This has changed the margins of the entire application.
...
:::note
Semantic tokens are terms used in design systems, referring to variables or tokens that abstract the design attributes of color and font size. These tokens are named so that their function and meaning can be intuitively understood from their names.
:::
...
Yamada UI supports the `loading animation` necessary for applications.
...
:::note status=warning
`custom` is not set in the [default config](/styled-system/configure/default-config). If you want to set up custom loading, please see [here](/styled-system/loading).
:::
...
:::note
The loading icon and style can be set with the `config` passed to the `useLoading` argument or `UIProvider`.
:::

:::note
If you want to know more about loading, please see [here](/styled-system/loading).
:::

### Using Notifications

Yamada UI supports `notifications` that are necessary for your application.

To display notifications, use `useNotice`. `useNotice` returns an instance that displays and controls notifications.
...
:::note
You can change the variant and display position of the notification with the `config` that can be passed to the arguments of `useNotice` or `UIProvider`.
:::

:::note
If you want to know more about notifications, please see [here](/styled-system/notification).
:::

## About Styles

With Yamada UI, you can easily change styles just by passing `props` to the components. It also provides many convenient shorthands, improving development efficiency.

For example, for `padding`, you can also write `p`, and if you want to set both `margin-left` and `margin-right`, you can write `mx`.
...
Yamada UI supports ready-to-use responsive styles. Just by passing an object to the style's `props`, it will support PC-first responsive styles.
...
:::note
If you want to change the breakpoints, please see [here](/styled-system/responsive-styles#customizing-breakpoints).
:::

## About Color Mode

Yamada UI includes support for managing the color mode of your application.
...
- `initialColorMode`: This is the initial value of the color mode. It accepts three values.
...
:::note
If you want to change the config, please see [here](/styled-system/configure/customize-config).
:::
...
:::note
The color mode is implemented using `localStorage` or `cookies`. If you want to learn about color modes, please see [here](/styled-system/color-mode).
:::
...
- `whileHover`: This is an animation that fires when an element is hovered over.
- `whileTap`: This is an animation that fires when an element is clicked or tapped.
- `whileFocus`: This is an animation that fires when an element is focused.

This time, we are setting `scale: 1.1` and `scale: 0.9` to enlarge and reduce the size.
...
"I'll be waiting for you in [Learn in Application](/getting-started/advanced) next///"😘

## Learn More

Why not learn more about Yamada UI, including its applications and themes? 😎
