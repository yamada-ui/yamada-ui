...
|                         | `aria-disabled` | `disabledRowIds`が設定されている場合は、`true`を設定します。                                                    |
|                         | `aria-rowcount` | テーブルの行数を示します。                                                                                      |
|                         | `aria-colcount` | テーブルの列数を示します。                                                                                      |
| `Thead` <Tag>内部</Tag> | `aria-sort`     | 表の項目が昇順の場合は`ascending`を設定し、降順の場合は`descending`を設定、指定がない場合は`none`を設定します。 |
|                         | `aria-colindex` | 列がテーブルの何列目であるかを示します。                                                                        |
|                         | `aria-rowindex` | 行がテーブルの何行目であるかを示します。                                                                        |
| `Tbody` <Tag>内部</Tag> | `aria-selected` | 行が選択済みの場合は、`true`を設定し、未選択の場合は`false`を設定します。                                       |
|                         | `aria-rowindex` | 行がテーブルの何行目であるかを示します。                                                                        |
