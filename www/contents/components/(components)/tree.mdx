...
|                     | `aria-activedescendant` | Sets the `id` of the currently focused item.                              |
...
|                     | `id`                    | Used to associate with `Tree.Root`.                                       |
...
|                     | `aria-labelledby`       | Sets the `id` of the related `Tree.Item > span`.                          |
| `Tree.Item > div`   | `aria-controls`         | Sets the `id` of the related `Tree.Item > ul`.                            |
| `Tree.Item > ul`    | `role="group"`          | Indicates that the element is a group.                                    |
|                     | `id`                    | Used to associate with `Tree.Item > div`.                                 |
|                     | `aria-labelledby`       | Sets the `id` of the related `Tree.Item > span`.                          |
|                     | `aria-busy`             | Set to `"true"` when the item's children are loading.                     |
| `Tree.Item > input` | `id`                    | Used to associate with the parent `Tree.Item > input`.                    |
|                     | `aria-controls`         | Sets the `id` of related child `Tree.Item > input` elements.              |
|                     | `aria-labelledby`       | Sets the `id` of the related `Tree.Item > span`.                          |
| `Tree.Item > span`  | `id`                    | Used to associate `Tree.Item`, `Tree.Item > ul`, and `Tree.Item > input`. |
