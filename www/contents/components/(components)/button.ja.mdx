...
:::tip
リンクとして機能させたい場合は、`as="a"`を設定します。フレームワークがリンクコンポーネントを提供している場合は、`as={FrameworkLink}`を設定します。各フレームワークのリンクコンポーネントとの統合は、[こちら](/docs/get-started#pick-your-framework)から使用しているフレームワークのドキュメントをご覧ください。
:::

:::tip
`Button`の子要素がアイコンのみの場合は、[IconButton](/docs/components/icon-button)を使用します。
:::

:::tip
`Button`が複数存在し、それらの`Button`がグルーピングが可能な場合は、[ButtonGroup](#grouping)を使用します。
:::

:::tip
閉じるボタンが必要な場合は、[CloseButton](/docs/components/close-button)の使用を検討してください。
:::

:::tip
ファイルを選択するボタンとして使用する場合は、[FileButton](/docs/components/file-button)の使用を検討してください。
:::
...
| -------------------- | --------------- | ------------------------------------------------------------------------------------------------------------ |
| `button.ui-button`   | `role="button"` | デフォルトでは`Button`は`button.ui-button`としてレンダリングされます。`as`を使うとホスト要素は変更されます。 |
