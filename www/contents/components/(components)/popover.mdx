...
`gutter` sets a simple distance from the element, while `offset` allows setting `[horizontal axis, vertical axis]`.
...
To use modal mode, set `modal` to `true`. This enables scroll blocking and focus trapping.
...
To focus a specific element when the popover opens, pass a `Ref` to `initialFocusRef`.
...
To block background scrolling while the popover is open, set `blockScrollOnMount` to `true`.
...
You can create nested popovers by placing one inside another.
...
return (
  <HStack>
    <Button onClick={onOpen}>External Control</Button>
