...
When using only icons in `SegmentedControl.Item`, set `aria-label` for the hidden radio input with `inputProps`.
...
| `SegmentedControl.Item` | `aria-disabled`      | Sets to `"true"` on the item label and hidden radio input if `disabled` is set.                  |
|                         | `readOnly`           | Sets the native `readOnly` attribute on the hidden radio input if `readOnly` is set.             |
|                         | `data-readonly`      | Sets on the item label and hidden radio input if `readOnly` is set.                              |

## Similar Components

<Similar name="segmented-control" />

## Uses Components & Hooks

<Uses name="segmented-control" />
