...
function getVersion(packageName: string): null | string {
  const raw = dependencies?.[packageName]
...
const extractImports = async (
  ...
  try {
    ...
    sourceFile.getImportDeclarations().forEach((importDeclaration) => {
      ...
      if (result === "external") {
        const packageName = parsePackageName(moduleSpecifier)
...
const generateDependenciesJson = async () => {
  ...
  await writeFile("dependencies.json", JSON.stringify(allDependencies, null, 2))
}
