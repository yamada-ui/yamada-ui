...
:::tip
ユーザーにタスクの手順を案内し、現在のステップを示したい場合は、[Steps](/docs/components/steps)を使用してください。`Timeline`は手順を示すためではなく、起きたことを振り返るのに適しています。
:::

:::tip
コネクターが不要な場合は、[List](/docs/components/list)や他のデータ表示コンポーネントのほうが読み取りやすいことがあります。
:::
