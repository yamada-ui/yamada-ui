...
React Routerの`Link`コンポーネントをYamada UIのスタイリングシステムと統合します。
これにより、Yamada UIのテーマやスタイルをReact Routerコンポーネントに適用できます。

### Link

`as` propを使用して、React Routerの`Link`とYamada UIの`Link`を統合します。
