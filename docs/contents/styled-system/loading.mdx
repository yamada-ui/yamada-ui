---
title: Loading
description: Yamada UI supports the necessary loading animations for applications.
order: 10
---

Yamada UI supports the necessary `loading animations` for applications.
...
:::note status=warning
`custom` is not set in the [default config](/styled-system/configure/default-config). If you want to set up custom loading, please see [here](#Using-custom-components).
:::
...
By passing options to the method, you can customize the loading.
...
`message` can take a `JSX element`.
...
When updating the message, pass the options using the `update` method.
...
By using `initialState`, a loading display will be shown when the application loads.

This is suitable for communication during loading (such as retrieving user information).
...
Depending on the scenario, you may need to implement a loading animation that matches your project.
...
:::note
If you want to learn more about configuration, please see [Customizing Configuration](/styled-system/configure/customize-config).
:::
