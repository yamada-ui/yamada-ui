...
| `input`              | `aria-checked`     | チェックボックスがチェックされている場合は`"true"`を設定し、チェックされていない場合は`"false"`を設定し、`indeterminate`が設定されている場合は`"mixed"`を設定します。                                |
...
| `CheckboxIndicator`  | `aria-hidden`      | 要素をアクセシビリティツリーから除外します。                                                                                                                                                         |
