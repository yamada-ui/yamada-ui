...
:::tip
`Popover`は、トリガーに関連する補足情報やコントロールを表示する場合に使用します。短い非インタラクティブな補足情報には[Tooltip](/docs/components/tooltip)を、操作を一時的に止める判断には[Modal](/docs/components/modal)を使用します。
:::
...
| `Shift` + `Tab` | ポップオーバー内の無効ではない前の要素にフォーカスします。最初の要素の場合はポップオーバーを閉じます。  | `open={true}`                       |
...
| 要素                         | ロールと属性             | 使い方                                                                                                                     |
| :--------------------------- | :----------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| `div.ui-popover__content`    | `role="dialog"`          | ダイアログであることを示します。                                                                                           |
|                              | `id`                     | `button.ui-popover__trigger`と関連付けるために使用する`id`。                                                               |
|                              | `aria-labelledby`        | 関連した`div.ui-popover__header`の`id`を設定します。                                                                       |
|                              | `aria-describedby`       | 関連した`div.ui-popover__body`の`id`を設定します。                                                                         |
|                              | `aria-hidden`            | ポップオーバーが開いている場合は`"false"`を設定し、閉じている場合は`"true"`を設定します。                                  |
|                              | `aria-modal`             | `modal={true}`の場合は`"true"`を設定します。                                                                               |
| `button.ui-popover__trigger` | `role="button"`          | ボタンであることを示します。                                                                                               |
|                              | `aria-disabled`          | `disabled`が設定されている場合は`"true"`を設定します。                                                                     |
|                              | `aria-expanded`          | ポップオーバーが開いている場合は`"true"`を設定し、閉じている場合は`"false"`を設定します。                                  |
|                              | `aria-controls`          | ポップオーバーが開いている場合は関連した`div.ui-popover__content`の`id`を設定し、閉じている場合は`undefined`を設定します。 |
|                              | `aria-haspopup="dialog"` | ダイアログをトリガーすることを示します。                                                                                   |
| `div.ui-popover__header`     | `id`                     | `div.ui-popover__content`と関連付けるために使用する`id`。                                                                  |
| `div.ui-popover__body`       | `id`                     | `div.ui-popover__content`と関連付けるために使用する`id`。                                                                  |
