...
- `ModalOverlay`: A component for the overlay that exists behind the modal.
- `ModalCloseButton`: A component for the button that closes the modal.
- `ModalHeader`: A component that displays the header of the modal.
- `ModalBody`: A component that displays the main content of the modal.
- `ModalFooter`: A component that displays the footer of the modal.
...
To change the show/hide animation, set `animation` to values like `top` or `left`. By default, `scale` is set.
...
By default, `inside` is set, and scrolling only occurs within the `ModalBody`. If you set `outside`, scrolling will occur within the `Modal`.
...
By default, when the modal opens, scrolling of the main content is blocked to ensure accessibility. If you do not want to block the scrolling of the main content, set `blockScrollOnMount` to `false`.
...
If you want to customize the `ModalOverlay` by passing `props`, etc., set it without omitting.
...
To disable (hide) the modal overlay, set `withOverlay` to `false`.
...
If you want to customize the `ModalCloseButton` by passing `props`, etc., set it without omitting.
