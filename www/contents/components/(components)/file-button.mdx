...
| ---------------- | ------------------------------------------------------- | ----- |
| `Enter`, `Space` | Activates the focused button and opens the file dialog. | -     |

### ARIA Roles and Attributes

| Component and Element   | Roles and Attributes | Usage                                                                                                                                                                                           |
| ----------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `button.ui-file-button` | `aria-describedby`   | If `FileButton` is within a `Field.Root` and `Field.Root` has an `errorMessage`, `helperMessage`, or a `Field.ErrorMessage`, `Field.HelperMessage`, sets its `id`.                              |
|                         | `aria-disabled`      | Sets to `"true"` in the readOnly focusable-disabled path (`readOnly` is set and `disabled` is not set). If `disabled` is set, disabled state is represented by the native `disabled` attribute. |
|                         | `aria-invalid`       | Sets to `"true"` if `invalid` is set.                                                                                                                                                           |
|                         | `aria-labelledby`    | If `FileButton` is within a `Field.Root` and `Field.Root` has a `label`, sets its `id`.                                                                                                         |
|                         | `aria-required`      | Sets to `"true"` if `required` is set.                                                                                                                                                          |

## Similar Components

<Similar name="file-button" />

## Uses Components & Hooks

<Uses name="file-button" />
