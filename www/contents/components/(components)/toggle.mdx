...
:::tip
If you need a one-time action like submitting a form or navigating, use [Button](/docs/components/button) instead of `Toggle`.
:::

:::tip
If there are multiple `Toggle` components that can be grouped together, use [ToggleGroup](#grouping).
:::
...
| ------------------------ | -------------------- | ------------------------------------------------------------------------- |
| `div.ui-toggle__group`   | `role="group"`       | Indicates that it's a group.                                              |
| `button.ui-toggle__root` | `aria-pressed`       | Set to `true` if the element is pressed, or `false` if it is not pressed. |
