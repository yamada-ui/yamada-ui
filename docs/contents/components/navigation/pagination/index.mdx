---
title: Pagination
description: "`Pagination` is a component for managing the pagination and navigation of content."
package_name: "@yamada-ui/pagination"
is_tabs: true
with_description: true
tab: Usage
---

## Import

```ts
import { Pagination } from "@yamada-ui/react"
```

## Usage

```tsx
<Pagination total={10} />
```

### Variants

```tsx
<VStack>
  <Pagination total={10} variant="solid" />
  <Pagination total={10} variant="outline" />
  <Pagination total={10} variant="ghost" />
  <Pagination total={10} variant="unstyled" />
</VStack>
```

### Add Edge Control Buttons

To add buttons to move to the first and last pages, set `withEdges` to `true`.

```tsx
<Pagination total={10} withEdges />
```

### Customize the Number of Siblings

If you want to change the number of pages displayed before and after the current page, set the desired number to `siblings`.

```tsx
<Pagination total={77} siblings={3} />
```

### Customize the Number of Boundaries

If you want to change the number of pages displayed at the beginning and end, set the desired number to `boundaries`.

```tsx
<Pagination total={77} boundaries={3} />
```

### Disable Pagination

If you want to disable pagination, set `isDisabled`.

```tsx
<VStack>
  <Pagination total={10} variant="solid" isDisabled />
  <Pagination total={10} variant="outline" isDisabled />
  <Pagination total={10} variant="ghost" isDisabled />
  <Pagination total={10} variant="unstyled" isDisabled />
</VStack>
```

### Customize Control Buttons

- `controlProps`: `props` set for both buttons.
- `controlPrevProps`: `props` set for the previous button.
- `controlNextProps`: `props` set for the next button.

```tsx
<VStack>
  <Pagination total={10} controlProps={{ children: <FontAwesomeIcon icon={faPoo} /> }} />
  <Pagination total={10} controlPrevProps={{ children: <FontAwesomeIcon icon={faPoo} /> }} />
  <Pagination total={10} controlNextProps={{ children: <FontAwesomeIcon icon={faPoo} /> }} />
</VStack>
```

### Customize Edge Control Buttons

- `edgeProps`: `props` set for both buttons.
- `edgeFirstProps`: `props` set for the first button.
- `edgeLastProps`: `props` set for the last button.

```tsx
<VStack>
  <Pagination total={10} withEdges edgeProps={{ children: <FontAwesomeIcon icon={faPoo} /> }} />
  <Pagination
    total={10}
    withEdges
    edgeFirstProps={{ children: <FontAwesomeIcon icon={faPoo} /> }}
  />
  <Pagination total={10} withEdges edgeLastProps={{ children: <FontAwesomeIcon icon={faPoo} /> }} />
</VStack>
```

### Control

```tsx functional=true
const [page, onChange] = useState<number>(1)

return <Pagination page={page} total={10} onChange={onChange} />
```
