...
`FileButton`は、アクセシビリティに関して[WAI-ARIA - Button Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/button/)に従います。
...
| キー             | 説明                                                                   | 状態 |
| ---------------- | ---------------------------------------------------------------------- | ---- |
| `Enter`, `Space` | フォーカスされたボタンをアクティブにし、ファイルダイアログを開きます。 | -    |

### ARIAロールと属性

| コンポーネントと要素    | ロールと属性       | 使い方                                                                                                                                                                                                 |
| ----------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `button.ui-file-button` | `aria-describedby` | `FileButton`が`Field.Root`内にあり、`Field.Root`に`errorMessage`、`helperMessage`、`Field.ErrorMessage`、`Field.HelperMessage`が設定されている場合は、その`id`を設定します。                           |
|                         | `aria-disabled`    | readOnlyのfocusable-disabled経路（`readOnly`が設定され、`disabled`が設定されていない場合）で`"true"`を設定します。`disabled`が設定されている場合、無効状態はネイティブの`disabled`属性で表現されます。 |
|                         | `aria-invalid`     | `invalid`が設定されている場合は`"true"`を設定します。                                                                                                                                                  |
|                         | `aria-labelledby`  | `FileButton`が`Field.Root`内にあり、`Field.Root`に`label`が設定されている場合は、その`id`を設定します。                                                                                                |
