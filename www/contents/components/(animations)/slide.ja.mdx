...
:::tip
Yamada UIでは関連する情報をグループ化して表示するコンポーネントとして、[`Drawer`](/docs/components/drawer)を提供しています。まず[`Drawer`](/docs/components/drawer)を使用できるか検討し、対応できないスタイリングがある場合に`Slide`を使用してください。
:::

### 配置を変更する

配置を変更する場合は、`placement`に`"block-start"`、`"block-end"`、`"inline-start"`、または`"inline-end"`を設定します。デフォルトでは、`"inline-end"`が設定されています。
