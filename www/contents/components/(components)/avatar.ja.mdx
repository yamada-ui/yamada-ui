...
| `div.ui-avatar__fallback` | `role="img"` | `fallbackMessage`が設定されていない場合、画像であることを示します。                                                                                            |
|                           | `aria-label` | `fallbackMessage`が設定されていない場合に設定します。`name`または`alt`が設定されている場合はその値を、設定されていない場合は`"アバターアイコン"`を設定します。 |
