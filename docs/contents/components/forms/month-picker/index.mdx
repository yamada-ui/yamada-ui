...
To change the default type, set `defaultType` to `month` or `year`. By default, `year` is set.
...
You can customize the method of parsing dates.
...
To change the format of the date, set a string to `inputFormat` or `yearFormat`.
...
By default, the calendar closes automatically upon selection. To prevent the calendar from closing on selection, set `closeOnSelect` to `false`.
