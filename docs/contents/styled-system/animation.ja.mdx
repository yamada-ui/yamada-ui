...
:::note
`Motion`コンポーネントは、内部的に[Framer Motion](https://www.framer.com/motion/)を使用しています。さらに詳しいコンポーネントの機能を知りたい場合は、[こちら](https://www.framer.com/motion/component/)をご覧ください。
:::
...
:::note status=warning
`initial`・`animate`・`exit`・`transition`で使われるスタイルオブジェクトは、Yamada UIの`Style Props`ではありません。スタイルオブジェクトのプロパティは、[Framer Motion](https://www.framer.com/motion/)のドキュメントをご覧ください。
:::
...
:::note
アニメーションをもっと知りたい場合は、[こちら](https://www.framer.com/motion/animation/)をご覧ください。
:::
...
:::note
ホバーのアニメーションをもっと知りたい場合は、[こちら](https://www.framer.com/motion/gestures/#hover)をご覧ください。
:::
...
:::note
クリック・タップのアニメーションをもっと知りたい場合は、[こちら](https://www.framer.com/motion/gestures/#tap)をご覧ください。
:::
...
:::note
フォーカスのアニメーションをもっと知りたい場合は、[こちら](https://www.framer.com/motion/gestures/#focus)をご覧ください。
:::
...
return (
  <Center ref={constraintsRef} h='md' gap='md'>
    <Motion
      drag
...
return (
  <Center ref={constraintsRef} h='md' p='md' gap='md'>
    <Motion
      drag
      dragConstraints={constraintsRef}
      dragElastic={0}
...
:::note
ドラッグのアニメーションをもっと知りたい場合は、[こちら](https://www.framer.com/motion/gestures/#drag)をご覧ください。
:::
...
return (
  <>
    ...
    <Box ref={constraintsRef} h='md' p='md' overflowY='auto'>
      <HStack mt='24rem' justifyContent='center'>
        <Motion
          initial={{ x: -100 }}
          whileInView={{ x: 0 }}
          viewport={{ root: constraintsRef }}
          transition={{ duration: 1 }}
        ...
        <Motion
          initial={{ x: 100 }}
          whileInView={{ x: 0 }}
          viewport={{ once: true, root: constraintsRef }}
          transition={{ duration: 1 }}
          onViewportEnter={(entry) => console.log('Scroll entires', entry)}
          onViewportLeave={(entry) => console.log('Scroll leaves', entry)}
          p='md'
...
```tsx functional=true
const constraintsRef = useRef<HTMLDivElement>(null)
const { scrollYProgress } = useScroll({ container: constraintsRef })
const scale = useTransform(scrollYProgress, [0, 1], [0.2, 2])

return (
  <Box ref={constraintsRef} position='relative' h='md' overflowY='auto'>
    <Box h='9xl'>
...
:::note
スクロールのアニメーションをもっと知りたい場合は、[こちら](https://www.framer.com/motion/scroll-animations/)をご覧ください。
:::
