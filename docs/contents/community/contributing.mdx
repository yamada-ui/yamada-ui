...
description: We want to create a community where Yamada UI is enjoyable and educational for anyone and everyone.
menu: Contributing
order: 2
---
...
4. Run `pnpm docs:dev` to start the local server. In the case of `Windows`, `contentlayer` may not work as expected. If so, run `pnpm docs:content` before `pnpm dev`.

The Yamada UI documentation site has over 800 pages, so you may feel a load on your development environment.
If you feel a load on your development environment, please select the necessary pages for development according to the following procedure.

5. Add `contentlayer.config.json`.
...
6. Change the contents of `contentlayer.config.json`.

- `contentDirInclude`: Include the specified directory.
- `contentDirExclude`: Exclude the specified directory.
...
- **`pnpm docs:start`**: Launches the production server.
