...
const {
  ...
  withContext,
  withProvider,
} = createSlotComponent<TooltipRootProps, TooltipStyle, TooltipRootProps>(
  "chart-tooltip",
  tooltipStyle,
)
...
export const Tooltip = withProvider(
  RechartsTooltip,
  "root",
)(({ colors, labelFormatter, unit, valueFormatter, ...props }) => {
...
const TooltipContent = withContext<"div", TooltipContentProps>(
  ({
    ...
    const items = payload.map(
      ({ name: nameProp, [dataKey]: valueProp, payload } = {}, index) => {
        const name = radialChart ? payload?.name : nameProp
        const color = colors?.find(({ dataKey }) => dataKey == name)?.color
