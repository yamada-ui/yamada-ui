...
:::note status=warning
[基本を学ぶ](/getting-started/installation/basic)や[応用を学ぶ](/getting-started/installation/advanced)など多くのページは、[すべて](/getting-started/installation)でインストールしている想定で説明されています。
:::
...
テーマをインストール後、アプリケーションのルートに`ThemeProvider`を追加し、`@yamada-ui/theme`から[デフォルトのテーマ](/styled-system/theming/default-theme)と[デフォルトのコンフィグ](/styled-system/configure/default-config)を渡します。もし、あなたのテーマやコンフィグがある場合は、そちらを渡します。
