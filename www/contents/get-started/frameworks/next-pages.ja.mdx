...
`Link`や`Image`などのNext.jsコンポーネントをYamada UIのスタイリングシステムと統合します。
これにより、Yamada UIのテーマやスタイルをNext.jsコンポーネントに適用できます。

### Link

`as` propを使用して、`next/link`とYamada UIの`Link`を統合します。
...
`styled`を使用して、`next/image`とYamada UIのスタイリングシステムを統合します。
...
`forwardProps`オプションは、Next.js固有のpropsをスタイルpropsとして扱わず、`Image`コンポーネントに渡します。
