...
To change colors, set a color in `series.color` or `lineProps.color`. The default is `"mono.500"`.
...
To change the type, set `type` or `lineProps.type` to `"monotone"`, `"linear"`, etc. The default is `"monotone"`.
...
To hide active dots, set `activeDot` or `lineProps.activeDot` to `false`. The default is `true`.
