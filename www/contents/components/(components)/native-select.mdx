...
You can omit `NativeSelectOption` by setting `items`.
...
To disable a specific option, set `disabled` to `true`.
...
Alternatively, you can set `disabled` to `true` in the item when using `items`.
