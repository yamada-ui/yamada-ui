...
```tsx preview
<VStack>
  <DatePicker gutter={32} />
  <DatePicker offset={[16, 16]} />
</VStack>
```
...
```tsx preview
<DatePicker placement="center-end" />
```
...
変更時にドロップダウンを開く場合は、`openOnChange`に関数を設定します。
...
アドオンを追加する場合は、`DatePicker`を`InputGroup`でラッピングし、`InputGroup.Addon`を使用します。
...
要素を追加する場合は、`DatePicker`を`InputGroup`でラッピングし、`InputGroup.Element`を使用します。
