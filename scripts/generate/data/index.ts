...
import ora from "ora"
import { format, writeFileWithFormat } from "packages/workspace/src/prettier"
    ...
    dirents.map(async (dirent) => {
      ...
      await Promise.all(
        dirents.map(async (dirent) => {
          ...
          const data: Data = { dependencies, dependents, sources }
...
async function generateIndexData() {
  ...
  await writeFileWithFormat(path.join(ENTRY_PATH, "data.json"), content, {
    parser: "json",
  })
}

async function publishData() {
  const filePaths = await glob(path.join(ENTRY_PATH, "**", "data.json"))
...
function main() {
  ...
  program
    .option("-p, --publish", "Publish the data")
    .action(async ({ publish = false }) => {
      ...
      await generateData(sourceMap, dependencyMap, dependentMap)
      await generateIndexData()

      spinner.succeed("Generated data")

      if (publish) {
        spinner.start("Publishing data")

        await publishData()

        spinner.succeed("Published data")
      }
