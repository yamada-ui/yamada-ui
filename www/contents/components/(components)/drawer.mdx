...
:::tip
Use `Drawer` for supplemental workflows or settings that can slide in while keeping the current page context. If users must focus on a short blocking task before continuing, use [Modal](/docs/components/modal).
:::
...
| -------------------------------- | ------------------------ | ------------------------------------------------------------------------------------ |
| `section.ui-drawer__content`     | `role="dialog"`          | Indicates that it is a dialog.                                                       |
...
| `.ui-drawer__trigger--open`      | `aria-expanded`          | Sets to `"true"` when modal is opened, and `"false"` when it is closed.              |
|                                  | `aria-controls`          | Sets the `id` of `section.ui-drawer__content` when open, and is not set when closed. |
|                                  | `aria-haspopup="dialog"` | Indicates that a dialog exists.                                                      |
|                                  | `aria-label`             | Sets to `"Open modal"`.                                                              |
| `button.ui-drawer__close-button` | `aria-label`             | Sets to `"Close modal"`.                                                             |
| `.ui-drawer__trigger--close`     | `aria-label`             | Sets to `"Close modal"`.                                                             |
| `div.ui-drawer__overlay`         | `aria-hidden`            | Excludes the element from the accessibility tree.                                    |
