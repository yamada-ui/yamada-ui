...
| ---------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `fieldset.ui-fieldset__root`       | `aria-labelledby`    | `Fieldset.Legend`と共有された`id`を参照して`fieldset`グループにラベルを設定します。`aria-labelledby`プロパティと組み合わせて使用されます。 |
| `span.ui-fieldset__helper-message` | `aria-describedby`   | 関連した`Fieldset.Legend`の`id`を設定します。                                                                                              |
| `span.ui-fieldset__error-message`  | `aria-live="polite"` | `invalid`が設定されている場合に設定し、フォーカスの外側で更新される可能性があることを示します。                                            |

## 類似のコンポーネント

<Similar name="fieldset" />
