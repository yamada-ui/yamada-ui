...
`CircleProgress.Root` renders the outer container. To provide a custom accessible name for the screen-reader progressbar, set `aria-label` or `aria-labelledby` on `CircleProgressCircle` through `circleProps`.
