...
:::tip
ファイル選択には、[FileInput](/docs/components/file-input)を使用します。
:::

:::tip
パスワードの入力には、[PasswordInput](/docs/components/password-input)を使用します。
:::

:::tip
ピンコードやOTPの入力には、[PinInput](/docs/components/pin-input)を使用します。
:::

:::tip
数値の入力には、[NumberInput](/docs/components/number-input)を使用します。
:::

:::tip
複数行のテキスト入力には、[Textarea](/docs/components/textarea)を使用します。
:::

:::tip
インラインで編集可能なテキスト入力には、[Editable](/docs/components/editable)を使用します。
:::
...
プレースホルダーの色を変更する場合は、`_placeholder`に値を設定します。
...
`Field.Root`を使用しない場合は、`Input`に`aria-label`または`aria-labelledby`を設定します。
...
| 要素             | ロールと属性       | 使い方                                                                                                                                                                                  |
| ---------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `input.ui-input` | `aria-invalid`     | `invalid`が設定されている場合は`"true"`を設定します。                                                                                                                                   |
|                  | `aria-disabled`    | `disabled`が設定されている場合は`"true"`を設定します。                                                                                                                                  |
|                  | `aria-describedby` | `Input`が`Field.Root`内にあり、`Field.Root`に`errorMessage`または`helperMessage`、もしくは`Field.ErrorMessage`または`Field.HelperMessage`が設定されている場合は、その`id`を設定します。 |
