...
:::tip
If you need to guide users through a task and show the current step, use [Steps](/docs/components/steps). `Timeline` is better for reviewing what happened, not for controlling a process.
:::

:::tip
If a connector is not needed, [List](/docs/components/list) or other data display components may be easier to read.
:::
