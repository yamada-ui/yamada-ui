---
title: Loading
description: "`Loading`は、データの読み込み中などの待機時間に表示するコンポーネントです。"
package_name: "@yamada-ui/loading"
order: 1
with_description: true
tab: Props
---

## Props

### LoadingProps

<PropsCard
  id="loading-colorscheme"
  name="colorScheme"
  type='"whiteAlpha" | "blackAlpha" | "gray" | "neutral" | "red" | "danger" | "rose" | "pink" | "flashy" | "orange" | "warning" | "amber" | "yellow" | "lime" | "green" | "success" | "emerald" | "teal" | "cyan" | "sky" | "blue" | "primary" | "info" | "link" | "indigo" | "violet" | "secondary" | "purple" | "fuchsia"'
  description={"The visual color appearance of the component."}
  defaultValue={"primary"}
/>

<PropsCard
  id="loading-duration"
  name="duration"
  type="string | number"
  description={"The CSS `dur` property."}
/>

<PropsCard
  id="loading-message"
  name="message"
  type="type ONLY_FOR_FORMAT =
  | string
  | number
  | boolean
  | ReactElement<any, string | JSXElementConstructor<any>>
  | Iterable<ReactNode>
  | ReactPortal"
/>

<PropsCard
  id="loading-secondarycolor"
  name="secondaryColor"
  type='UIValue<"border" | "link" | "primary" | "secondary" | "info" | "success" | "warning" | "danger" | Color | "current" | "focus" | "whiteAlpha.50" | "whiteAlpha.100" | "whiteAlpha.200" | ... 316 more ... | "link.950">'
  description={"The CSS `color` property."}
  defaultValue={"'primary'"}
/>

<PropsCard
  id="loading-size"
  name="size"
  type="string"
  description={"The size of the Loading."}
  defaultValue={"1em"}
/>

<PropsCard
  id="loading-variant"
  name="variant"
  type="string"
  description={"The variant of the Loading."}
  defaultValue={"oval"}
/>
