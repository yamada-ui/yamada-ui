...
return (
  <Wrap gap='md'>
    ...
    <Button onClick={() => background.start({ duration: 5000 })}>Start background loading</Button>
  </Wrap>
)
```
...
function App() {
  ...
  return <Button onClick={() => custom.start({ duration: 5000 })}>Start custom loading</Button>
}
