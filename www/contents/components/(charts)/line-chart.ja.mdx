...
カラーを変更する場合は、`series.color`または`lineProps.color`に色を設定します。デフォルトは、`"mono.500"`です。
...
タイプを変更する場合は、`type`または`lineProps.type`に`"monotone"`や`"linear"`などを設定します。デフォルトは、`"monotone"`です。
...
アクティブドットを非表示にする場合は、`activeDot`または`lineProps.activeDot`を`false`に設定します。デフォルトは、`true`です。
