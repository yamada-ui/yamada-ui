---
title: NativePopover
description: "`NativePopover` is a component that floats around an element to display information using the HTML Popover API."
tags: ["overlay", "floating"]
storybook: components-native-popover--basic
source: components/native-popover
style: components/native-popover/native-popover.style.ts
---

```tsx preview
<NativePopover.Root>
  <NativePopover.Trigger>
    <Button>Click Me</Button>
  </NativePopover.Trigger>

  <NativePopover.Content>
    <NativePopover.Header>ベジータ!</NativePopover.Header>
    <NativePopover.Body>
      がんばれカカロット……お前がナンバー１だ！！
    </NativePopover.Body>
  </NativePopover.Content>
</NativePopover.Root>
```

:::note
`NativePopover` uses the HTML Popover API, which provides better performance and accessibility compared to the [Popover](/components/popover). However, it requires browser support for the Popover API.
:::

## Usage

:::code-group

```tsx [package]
import { NativePopover } from "@yamada-ui/react"
```

```tsx [alias]
import { NativePopover } from "@/components/ui"
```

```tsx [monorepo]
import { NativePopover } from "@workspaces/ui"
```

:::

```tsx
<NativePopover.Root>
  <NativePopover.Anchor />
  <NativePopover.Trigger />
  <NativePopover.Content>
    <NativePopover.Header />
    <NativePopover.Body />
    <NativePopover.Footer />
  </NativePopover.Content>
</NativePopover.Root>
```

### Change Size

```tsx preview
<Wrap gap="md">
  <For each={["xs", "sm", "md", "lg"]}>
    {(size) => (
      <NativePopover.Root key={size} size={size}>
        <NativePopover.Trigger>
          <Button>Size ({size})</Button>
        </NativePopover.Trigger>

        <NativePopover.Content>
          <NativePopover.Header>ベジータ!</NativePopover.Header>
          <NativePopover.Body>
            がんばれカカロット……お前がナンバー１だ！！
          </NativePopover.Body>
        </NativePopover.Content>
      </NativePopover.Root>
    )}
  </For>
</Wrap>
```

### Add Footer

```tsx preview
<NativePopover.Root>
  <NativePopover.Trigger>
    <Button>Click Me</Button>
  </NativePopover.Trigger>

  <NativePopover.Content>
    <NativePopover.Header>ベジータ!</NativePopover.Header>
    <NativePopover.Body>
      がんばれカカロット……お前がナンバー１だ！！
    </NativePopover.Body>
    <NativePopover.Footer>第280話</NativePopover.Footer>
  </NativePopover.Content>
</NativePopover.Root>
```

### Close Trigger

You can add a close button inside the popover using `NativePopover.CloseTrigger`.

```tsx preview
<NativePopover.Root>
  <NativePopover.Trigger>
    <Button>Click Me</Button>
  </NativePopover.Trigger>

  <NativePopover.Content>
    <NativePopover.Header>ベジータ!</NativePopover.Header>
    <NativePopover.Body>
      がんばれカカロット……お前がナンバー１だ！！
    </NativePopover.Body>
    <NativePopover.Footer>
      <NativePopover.CloseTrigger>
        <Button>Close</Button>
      </NativePopover.CloseTrigger>
    </NativePopover.Footer>
  </NativePopover.Content>
</NativePopover.Root>
```

### Display Popover on Different Element

To display a popover on a different element, use `NativePopover.Anchor`.

```tsx preview
<NativePopover.Root>
  <Wrap gap="md">
    <NativePopover.Anchor>
      <Center borderWidth="1px" h="10" px="3" rounded="l2">
        Display Popover Here
      </Center>
    </NativePopover.Anchor>

    <NativePopover.Trigger>
      <Button>Click Me</Button>
    </NativePopover.Trigger>
  </Wrap>

  <NativePopover.Content>
    <NativePopover.Header>ベジータ!</NativePopover.Header>
    <NativePopover.Body>
      がんばれカカロット……お前がナンバー１だ！！
    </NativePopover.Body>
  </NativePopover.Content>
</NativePopover.Root>
```

### Change Placement

To change the placement, set `placement` to `"start"`, `"end-end"`, etc. By default, `"end"` is set.

```tsx preview
<Wrap gap="md">
  <For
    each={[
      "start",
      "start-start",
      "start-end",
      "start-center",
      "end",
      "end-start",
      "end-end",
      "end-center",
      "center-start",
      "center-start-start",
      "center-start-end",
      "center-end",
      "center-end-start",
      "center-end-end",
    ]}
  >
    {(placement) => (
      <NativePopover.Root key={placement} placement={placement}>
        <NativePopover.Trigger>
          <Button>{toTitleCase(placement)}</Button>
        </NativePopover.Trigger>

        <NativePopover.Content>
          <NativePopover.Header>ベジータ!</NativePopover.Header>
          <NativePopover.Body>
            がんばれカカロット……お前がナンバー１だ！！
          </NativePopover.Body>
        </NativePopover.Content>
      </NativePopover.Root>
    )}
  </For>
</Wrap>
```

### Change Offset

To change the offset, set `gutter` or `offset` to a value.

```tsx preview
<HStack>
  <NativePopover.Root gutter={32}>
    <NativePopover.Trigger>
      <Button>Gutter</Button>
    </NativePopover.Trigger>

    <NativePopover.Content>
      <NativePopover.Header>ベジータ!</NativePopover.Header>
      <NativePopover.Body>
        がんばれカカロット……お前がナンバー１だ！！
      </NativePopover.Body>
    </NativePopover.Content>
  </NativePopover.Root>

  <NativePopover.Root offset={[16, 16]}>
    <NativePopover.Trigger>
      <Button>Offset</Button>
    </NativePopover.Trigger>
    <NativePopover.Content>
      <NativePopover.Header>ベジータ!</NativePopover.Header>
      <NativePopover.Body>
        がんばれカカロット……お前がナンバー１だ！！
      </NativePopover.Body>
    </NativePopover.Content>
  </NativePopover.Root>
</HStack>
```

### Popover Modes

The HTML Popover API supports different [modes](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/popover#value) for controlling popover behavior. This is a key feature that distinguishes `NativePopover` from the `Popover` component.

```tsx preview
<Wrap gap="md">
  <NativePopover.Root popover="auto">
    <NativePopover.Trigger>
      <Button>Click Me</Button>
    </NativePopover.Trigger>

    <NativePopover.Content>
      <NativePopover.Header>ベジータ!</NativePopover.Header>
      <NativePopover.Body>
        がんばれカカロット……お前がナンバー１だ！！
      </NativePopover.Body>
    </NativePopover.Content>
  </NativePopover.Root>

  <NativePopover.Root popover="hint">
    <NativePopover.Trigger>
      <Button>Click Me</Button>
    </NativePopover.Trigger>

    <NativePopover.Content>
      <NativePopover.Header>ベジータ!</NativePopover.Header>
      <NativePopover.Body>
        がんばれカカロット……お前がナンバー１だ！！
      </NativePopover.Body>
    </NativePopover.Content>
  </NativePopover.Root>

  <NativePopover.Root popover="manual">
    <NativePopover.Trigger>
      <Button>Click Me</Button>
    </NativePopover.Trigger>

    <NativePopover.Content>
      <NativePopover.Header>ベジータ!</NativePopover.Header>
      <NativePopover.Body>
        がんばれカカロット……お前がナンバー１だ！！
      </NativePopover.Body>
    </NativePopover.Content>
  </NativePopover.Root>
</Wrap>
```

### Disable

To disable the popover, set `disabled` to `true`.

```tsx preview
<NativePopover.Root disabled>
  <NativePopover.Trigger>
    <Button>Click Me</Button>
  </NativePopover.Trigger>

  <NativePopover.Content>
    <NativePopover.Header>ベジータ!</NativePopover.Header>
    <NativePopover.Body>
      がんばれカカロット……お前がナンバー１だ！！
    </NativePopover.Body>
  </NativePopover.Content>
</NativePopover.Root>
```

### Nest Popovers

```tsx preview
<NativePopover.Root>
  <NativePopover.Trigger>
    <Button>Open Parent</Button>
  </NativePopover.Trigger>

  <NativePopover.Content>
    <NativePopover.Header>ベジータ!</NativePopover.Header>
    <NativePopover.Body>
      <Text>がんばれカカロット……お前がナンバー１だ！！</Text>

      <NativePopover.Root>
        <NativePopover.Trigger>
          <Button>Click Me</Button>
        </NativePopover.Trigger>

        <NativePopover.Content>
          <NativePopover.Header>ベジータ!</NativePopover.Header>
          <NativePopover.Body>
            がんばれカカロット……お前がナンバー１だ！！
          </NativePopover.Body>
        </NativePopover.Content>
      </NativePopover.Root>
    </NativePopover.Body>
  </NativePopover.Content>
</NativePopover.Root>
```

## Props

<PropsTable name="native-popover" />

## Accessibility

### Keyboard Navigation

| Key      | Description         | State                              |
| -------- | ------------------- | ---------------------------------- |
| `Escape` | Closes the popover. | `popover="auto"`, `popover="hint"` |

### ARIA Roles and Attributes

| Element                             | Roles and Attributes     | Usage                                                             |
| ----------------------------------- | ------------------------ | ----------------------------------------------------------------- |
| `div.ui-native-popover__content`    | `role="dialog"`          | Indicates that it is a dialog.                                    |
|                                     | `id`                     | Used to associate with `button.ui-native-popover__trigger`.       |
|                                     | `aria-labelledby`        | Sets the `id` of the associated `div.ui-native-popover__header`.  |
|                                     | `aria-describedby`       | Sets the `id` of the associated `div.ui-native-popover__body`.    |
| `button.ui-native-popover__trigger` | `role="button"`          | Indicates that it is a button.                                    |
|                                     | `aria-haspopup="dialog"` | Indicates that it triggers a dialog.                              |
|                                     | `aria-controls`          | Sets the `id` of the associated `div.ui-native-popover__content`. |
|                                     | `aria-disabled`          | Sets to `"true"` when `disabled={true}`.                          |
| `div.ui-native-popover__header`     | `id`                     | Used to associate with `div.ui-native-popover__content`.          |
| `div.ui-native-popover__body`       | `id`                     | Used to associate with `div.ui-native-popover__content`.          |

## Similar Components

<Similar name="native-popover" />

## Used By Components & Hooks

<UsedBy name="native-popover" />
