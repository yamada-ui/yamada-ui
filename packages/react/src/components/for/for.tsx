...
function* generateItems<T>(each: T[], options: ForGeneratorOptions<T>) {
  const {
    children,
    filterBy,
    limitBy,
...
export const For = <T,>(props: ForProps<T>): ReactNode => {
  const { each, fallback, ...generatorProps } = props
  if (!each || !isArray(each) || !each.length) return fallback || null

  return Array.from(generateItems(each, generatorProps))
}
