...
To display a footer, set `withFooter` to `true`. If you want to display checkboxes in the footer similar to the header, set `withFooterSelect` to `true`.
...
To add default sorting, set `defaultSort` with `id` and `desc`.
...
By default, multi-sort is enabled. If you want to limit the maximum number of multi-sort selections, set a number to `maxMultiSortColCount`.
...
### Change Ascending/Descending Sort Order
...
The default row ID is the string `index`. For the first row, it will be `"0"`. If you want to use a specific column's value as the ID, set `rowId` to the value of `accessorKey` or `accessorFn` set in the column (object).
...
To set default selected rows, set `defaultSelectedRowIds` with an array of row IDs.
...
To disable selection for specific rows, set `defaultSelectedRowIds` with an array of row IDs.
...
### Enable Selection by Clicking Rows

To enable selection by clicking rows, set `rowsClickSelect` to `true`.
...
To customize the styling of checkboxes (column), set `props` to `checkboxProps` or `selectColumnProps`.
...
To customize styling, set `props` to `theadProps`, `rowProps`, etc.
