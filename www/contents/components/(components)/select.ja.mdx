...
`Field.Root`を使用しない場合は、説明的な`placeholder`を設定します。`placeholder`は`SelectField`トリガーの`aria-label`に設定されます。
...
`Field.Root`を使用する場合は、`Field.Root`のラベルの`id`が`SelectField`トリガーの`aria-labelledby`に設定されます。
...
|                    | `aria-label`              | `SelectField`トリガーに`placeholder`の値を設定します。                                                                                                                                         |
|                    | `aria-labelledby`         | `Select.Root`が`Field.Root`内にあり、`Field.Root`に`label`、もしくは`Field.Label`が設定されている場合は、その`id`を`SelectField`トリガーに設定します。                                         |
