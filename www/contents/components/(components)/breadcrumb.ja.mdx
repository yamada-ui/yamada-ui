...
```tsx [monorepo]
import { Breadcrumb } from "@workspace/ui"
```
...
区切りをカスタマイズするには、`separator`に区切り文字を設定します。
...
アイコンを区切りとして使用するには、`separator`に`ReactNode`を設定します。
...
省略を表示したい位置に`Breadcrumb.Ellipsis`を挿入します。
