...
:::tip
If there are multiple `Avatar` components that can be grouped together, use [AvatarGroup](#grouping).
:::

:::tip
If you don't need user-specific features like initials or fallback icons, use [Image](/docs/components/image) instead.
:::
...
| ------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `div.ui-avatar__group`    | `role="group"`       | Indicates that it is a group.                                                                                                                                                    |
| `img.ui-avatar__image`    | `alt`                | Alternative text for the avatar image. The component uses the `name` prop as the value for the `alt` attribute; if `name` is not provided, it uses the `alt` prop value instead. |
| `div.ui-avatar__fallback` | `role="img"`         | When `fallback` is not set, indicates that this is an image.                                                                                                                     |
|                           | `aria-label`         | When `fallback` is not set, sets the value of `name` or `alt` if provided. Otherwise, sets `"Avatar Icon"`.                                                                      |
