...
:::tip
通知としてではなく、テキストコンテンツ内にメモやヒントなどを視覚的に分かりやすく表示したい場合は、[Alert](/docs/components/alert)を使用します
:::

:::tip
フォームなどの通知をコンポーネント内の要素として表示したい場合は、[Snacks](/docs/components/snacks)を使用します。
:::
...
配置を変更する場合は、`placement`に`"start"`や`"end"`などを設定します。
...
通知を常に展開する場合は、`expand`に`true`を設定します。
