...
- **Require review from specific teams**: At least one approval from each of the `prime` and `standard` teams is required. `hirotomoyamada` belongs to both teams, so a single approval from `hirotomoyamada` satisfies both. `codex-for-yamada-ui` is in the `prime` team and `claude-for-yamada-ui` is in the `standard` team, so when bots review, both bot approvals are required together to satisfy the team requirements.
