...
Yamada UI includes support for managing the color mode of the application.
...
In order for the color mode to work properly, you need to add `ColorModeScript` within the `body`.
...
:::note
For the case of App Router, please see [here](/getting-started/frameworks/nextjs/app-router#uiprovidercacheprovidercolormodescriptを追加する).
:::
...
## Syntax of Color Mode

When setting the style of the color mode, it can be made compatible with the color mode by simply passing an array to the `props` of the style.
