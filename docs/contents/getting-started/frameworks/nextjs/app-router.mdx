...
```tsx live=false title=layout.tsx highlight={11}
...
```tsx live=false title=layout.tsx highlight={8,18,19,20,21,22,24}
import {
  colorModeManager,
  UIProvider,
  ColorModeScript,
...
:::note status=warning
Please pass the same `initialColorMode` to `props` as in your config. If you have your own config, pass its `initialColorMode`.
:::
...
:::note status=warning
You need to add `'use client'` at the top of the page.
:::
